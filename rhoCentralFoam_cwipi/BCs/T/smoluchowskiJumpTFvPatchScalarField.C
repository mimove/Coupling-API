...
{
    if (
        mag(accommodationCoeff_) < SMALL || mag(accommodationCoeff_) > 2.0)
    {
...
void Foam::smoluchowskiJumpTFvPatchScalarField::updateCoeffs()
{
    ...
    Field<scalar> C2(
        pmu / prho * sqrt(ppsi * constant::mathematical::piByTwo) * 2.0 * gamma_ / Pr.value() / (gamma_ + 1.0) * (2.0 - accommodationCoeff_) / accommodationCoeff_);
