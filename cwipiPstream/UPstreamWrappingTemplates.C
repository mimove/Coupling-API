    ...
    label *requestID)
{
    ...
    if (
        (sendCounts.size() != np || sendOffsets.size() < np) || (recvCounts.size() != np || recvOffsets.size() < np))
    {
        FatalErrorInFunction
    ...
    label *requestID)
{
    ...
    if (
        UPstream::master(comm) && (recvCounts.size() != np || recvOffsets.size() < np))
    {
    ...
    label *requestID)
{
    ...
    if (
        UPstream::master(comm) && (sendCounts.size() != np || sendOffsets.size() < np))
    {
        // Note: allow offsets to be e.g. 1 larger than nProc so we
        // can easily loop over the result
